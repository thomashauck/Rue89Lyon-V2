...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<!-- données avant la photo "image à la une" -->
	
	<p><?php echo get_the_date(); ?>&nbsp<?php _e( 'by', 'yoko' ); ?>&nbsp<?php the_author() ?>&nbsp | &nbsp<?php comments_popup_link( __( '0 comments', 'yoko' ), __( '1 Comment', 'yoko' ), __( '% Comments', 'yoko' ) ); ?></p>
		<?php if ( has_post_thumbnail() ): ?>
		<a href="<?php the_permalink(); ?>"></a><?php the_post_thumbnail('thumbnail'); ?><!-- champs images au dessus du titre -->
		<?php endif; ?>
		
		<?php $images= get_post_meta($post->ID, 'images', $single = true) ?>
